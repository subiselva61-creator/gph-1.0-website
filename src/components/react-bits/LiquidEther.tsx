...
export interface LiquidEtherProps {
  ...
  /** When true (default), auto-demo pauses while the pointer is over the canvas. Set false for full-viewport backgrounds so motion continues on idle hover. */
  pauseAutoOnHover?: boolean;
}
...
`;
    ...
    void main(){
    vec2 vel = texture2D(velocity, uv).xy;
    // Mild boost so residual flow still reads without neon overdrive
    float lenv = clamp(pow(length(vel) * 1.15, 0.75), 0.0, 1.0);
...
`;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: any) {
        ...
        this.autoDriver = new AutoDriver(Mouse, this as any, {
          ...
          pauseOnHover: props.pauseAutoOnHover
        });
        this.init();
  ...
  useEffect(() => {
    ...
    if (webgl.autoDriver) {
      ...
      webgl.autoDriver.pauseOnHover = pauseAutoOnHover;
